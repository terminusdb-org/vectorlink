 ...
 *            ancestor; `TerminusDB-Data-Version` reports the SERVED commit
 *            (≠ requested ⇒ stale). Never blocks.
 ...
 *            ancestry → the next search no longer 404s.
 *
...
describe("P3-LAG — lag, catch-up, negative cache", function () {
  ...
  it("P3-LAG-1: search a not-yet-indexed commit serves the nearest ancestor and flags stale", async function () {
    // Request a commit that was NEVER pushed (lag_c1). The engine must serve
    // c0 (the branch's last-indexed = nearest ancestor) and report it.
    const res = await agent()
      .get("/search")
      .query({ domain: DOMAIN, commit: "lag_c1_never_pushed", q: "wise old man", mode: "vector" })
      .set("Authorization", authHeader())
      .expect(200)
  ...
  it("P3-LAG-3: a push to the branch ancestry invalidates the 404 negative cache", async function () {
    ...
    // Now a search at the (still-unindexed) head resolves via the nearest
    // ancestor rc_c0 — no longer 404.
    const after = await agent()
      .get("/search")
      .query({ domain: RECOVER_DOMAIN, commit: "rc_head", q: "Jedi", mode: "vector" })
      .set("Authorization", authHeader())
