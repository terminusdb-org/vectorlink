...
// Strip an IRI back to its raw id (last path segment).
...
async function ioGatherSearch(ds, abt, buy, { k, queryDelayMs }) {
  // The snapshot holds BOTH catalogues, so a /search hit list is mixed; we
  // over-fetch (k plus the other side's expected interlopers) and then keep only
  // the opposite-side hits, taking the nearest k of those. overFetch is generous
  // because a record's nearest neighbours include its OWN-side near-twins.
  const overFetch = Math.max(k * 4, k + 20);
...
async function ioGatherDuplicates(ds, abt, buy, { threshold }) {
  ...
  log(`duplicates mode: set=Abt target=Buy then set=Buy target=Abt (bulk, stored vectors, top-1/dir)`);
  const abtGroups = await ioDuplicates({
    domain: ds.domain,
    commit: ds.commit,
    threshold,
    setDocTypes: ["Abt"],
  ...
  const buyGroups = await ioDuplicates({
    domain: ds.domain,
    commit: ds.commit,
    threshold,
    setDocTypes: ["Buy"],
