...
- **Cache configuration.** Lance's index cache defaults to 6 GiB; we use 2 GiB as a balanced default (`VECTORLINK_LANCE_INDEX_CACHE_BYTES`). The metadata cache defaults to 1 GiB in Lance; we use 512 MiB (`VECTORLINK_LANCE_METADATA_CACHE_BYTES`). With fragmented indices (many small deltas), a larger cache allows more index data to stay in memory, reducing disk I/O during search. If search latency is high after compaction, increasing the index cache is the first knob to turn.
...
make dev             # incremental debug build → target/debug/vectorlink
...
When the vectorlink binary is rebuilt with indexing changes (new index cadence, compaction logic, or vector index config), existing domains must be reindexed to pick up the new architecture. The reindex is triggered through the TerminusDB indexer plugin, which wipes the branch index on vectorlink and replays all commits from the oldest forward.
...
- `engine.documents_sent`: total documents pushed to vectorlink
- `engine.searchable_documents`: documents available for search
- `last_indexed_commit`: the most recent commit tagged in vectorlink
...
du -sh /tmp/vectorlink-data/admin__<org>__<db>.lance/
```
...
The E2E test (`tests/e2e/indexer-push-e2e.js` in the terminusdb repo) exercises the full pipeline: schema with embedding metadata, document insertion, automatic indexer push, and search through both the TerminusDB proxy and vectorlink directly.

```bash
make server-start
cd ../terminusdb
TERMINUSDB_BASE_URL=http://127.0.0.1:7373 \
VECTORLINK_URL=http://127.0.0.1:7372 \
...
src/
  ...
  bin/load.rs        Standalone bulk-load binary (vectorlink-load)
...
| `VECTORLINK_DIM` | `768` | Embedding vector dimension |
| `VECTORLINK_EMBED_BATCH_SIZE` | `32` | Texts per embedding HTTP call |
| `VECTORLINK_EMBED_CACHE_SIZE` | `20000` | Max embed cache entries (`none` to disable) |
| `VECTORLINK_TOKENIZER_PATH` | `/data/tokenizer.json.bz2` | Tokenizer file (bz2 or plain JSON) |
| `VECTORLINK_ADMIN_USER` | `admin` | HTTP Basic auth user |
| `VECTORLINK_ADMIN_PASSWORD` | `root` | HTTP Basic auth password |

## TerminusDB integration

TerminusDB pushes to vectorlink via the indexer plugin. Set these in the TerminusDB environment:

| Variable | Default | Description |
|---|---|---|
| `TERMINUSDB_INDEXER_BACKEND` | `http_vectorlink` | Indexer backend |
| `TERMINUSDB_VECTORLINK_ENDPOINT` | `http://127.0.0.1:7372` | Push target URL |
